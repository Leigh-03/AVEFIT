...
import { ChevronDown, ChevronUp, Dumbbell, Trash2, Plus, Users, AlertTriangle } from "lucide-react";
...
function MemberRow({ member, exercises, onChanged }) {
  ...
  const toList = (val) => {
    ...
    if (typeof val === "string") {
      // Handles comma-separated text, or Postgres array literal like {a,b}
      return val.replace(/^\{|\}$/g, "").split(",").map((s) => s.trim()).filter(Boolean);
    }
    return [];
  };

  const preferredDaysList = toList(member.preferred_days);
  const dayOptions = preferredDaysList.length > 0 ? preferredDaysList : DAYS;
  const injuries = toList(member.injuries);
  const healthConditions = toList(member.health_conditions);
  const hasHealthFlags = injuries.length > 0 || healthConditions.length > 0;
  const currentWeight = member.latest_weight ?? member.weight;
  const currentBmi = member.latest_bmi;
    ...
    <div className="border border-slate-800 rounded-2xl overflow-hidden bg-slate-900">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-white text-sm flex items-center gap-1.5">
              ...
              {hasHealthFlags && (
                <AlertTriangle size={13} className="text-amber-400" aria-label="Has injuries or health conditions on file" />
              )}
            </p>
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-slate-800">
          {/* Member details */}
          <div className="pt-4">
            <p className="text-xs font-semibold text-slate-500 mb-2">Member Details</p>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
              <div className="bg-slate-800/60 rounded-lg px-3 py-2">
                <p className="text-slate-500">Age / Gender</p>
                <p className="text-slate-200 font-medium mt-0.5">{member.age || "—"}{member.gender ? ` · ${member.gender}` : ""}</p>
              </div>
              <div className="bg-slate-800/60 rounded-lg px-3 py-2">
                <p className="text-slate-500">Height / Weight</p>
                <p className="text-slate-200 font-medium mt-0.5">{member.height ? `${member.height}cm` : "—"}{currentWeight ? ` · ${currentWeight}kg` : ""}</p>
              </div>
              <div className="bg-slate-800/60 rounded-lg px-3 py-2">
                <p className="text-slate-500">Target Weight</p>
                <p className="text-slate-200 font-medium mt-0.5">{member.target_weight ? `${member.target_weight}kg` : "—"}</p>
              </div>
              <div className="bg-slate-800/60 rounded-lg px-3 py-2">
                <p className="text-slate-500">Current BMI</p>
                <p className="text-slate-200 font-medium mt-0.5">{currentBmi || "—"}</p>
              </div>
              <div className="bg-slate-800/60 rounded-lg px-3 py-2">
                <p className="text-slate-500">Activity Level</p>
                <p className="text-slate-200 font-medium mt-0.5">{member.activity_level || "—"}</p>
              </div>
              <div className="bg-slate-800/60 rounded-lg px-3 py-2">
                <p className="text-slate-500">Intensity</p>
                <p className="text-slate-200 font-medium mt-0.5">{member.intensity ? `${member.intensity}/10` : "—"}</p>
              </div>
              <div className="bg-slate-800/60 rounded-lg px-3 py-2">
                <p className="text-slate-500">Days / Week</p>
                <p className="text-slate-200 font-medium mt-0.5">{member.workout_days_per_week || "—"}</p>
              </div>
              <div className="bg-slate-800/60 rounded-lg px-3 py-2">
                <p className="text-slate-500">Session Length</p>
                <p className="text-slate-200 font-medium mt-0.5">{member.workout_duration || "—"}</p>
              </div>
            </div>

            {hasHealthFlags && (
              <div className="mt-2 bg-amber-500/10 border border-amber-500/30 rounded-lg px-3 py-2.5">
                <p className="text-amber-400 text-xs font-semibold flex items-center gap-1.5 mb-1.5">
                  <AlertTriangle size={13} /> Health &amp; Safety Notes
                </p>
                {injuries.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mb-1.5">
                    {injuries.map((item, i) => (
                      <span key={i} className="bg-amber-500/15 text-amber-300 text-[11px] px-2 py-0.5 rounded-full">
                        Injury: {item}
                      </span>
                    ))}
                  </div>
